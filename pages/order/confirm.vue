<template>
	<s-layout title="确认订单">
		...
		<view class="bg-white address-box ss-m-b-14 ss-r-b-10" @tap="onSelectAddress" v-if="state.deliveryType !=3">
...
<script setup>
	...
	async function submitOrder() {
		const {
			...
			deliveryType: state.deliveryType, // TODO 芋艿：需要支持【门店自提】
	...
	onLoad(async (options) => {
		...
		state.deliveryType = state.orderPayload.deliveryType;
		await getOrderInfo();
		await getCoupons();
	});
</script>
